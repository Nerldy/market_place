...
test('Sign in error', async t => {
	...
	// click button
	await t.click(signInButton).wait(waitTime)
	...
	await t
		.expect(Selector(errorMessage).textContent)
		.eql('User not found. Please sign up')
})
...
test('Register new user', async t => {
	...
	// click button
	await t.click(registerButton).wait(5000) // wait 5 seconds
	...
	const refferalCodeInput = Selector('.input').withAttribute(
		'name',
		'referrer_code'
	)
	const emailInput = Selector('.input').withAttribute('type', 'email')
	const submitRegisterButton = Selector('button').withAttribute(
		'type',
		'submit'
	)
	...
	// input first name
	await t.click(firstNameInput).typeText(firstNameInput, firstName)

	// input last name
	await t.pressKey('tab').typeText(lasttNameInput, lastName)

	// input email
	await t.click(emailInput).typeText(emailInput, email)
	...
	// enter password
	await t.click(passwordNameInput).typeText(passwordNameInput, password)

	// enter refferal code
	await t.click(refferalCodeInput).typeText(refferalCodeInput, referralCode)

	// subMitForm()
	await t.click(submitRegisterButton)
	await t.wait(waitTime)

	// assert Congratulations message is displayed after sign up
	await t
		.expect(Selector('h2').withText('Congratulations!').exists)
		.ok('Congratulations! is displayed')
})
...
test('Sign in user', async t => {
	...
	await t
		.expect(Selector('.profile-menu').child(1).innerText)
		.contains('Sign Out')
})
